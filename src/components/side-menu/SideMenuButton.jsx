...
function SideMenuButton({ children, text, sectionID, sendDataToSideMenu, url, onClick, ...props }) {
    ...
    return (
        <>
            <Link to={url}>
                <button className='flex flex-nowrap justify-start py-3 items-center group transition duration-medium overflow-hidden lg:hover:bg-gray-300 group lg:selected:bg-gray-300 lg:focus:outline-hidden w-full'{...props} onClick={onClick}>
                    {children}
